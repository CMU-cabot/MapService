...
APP_SERVER_DIR="$ROOT_DIR/.build/cabot-app-server"
...
(cd "$APP_SERVER_DIR" && docker compose -f docker-compose.yaml down >/dev/null 2>&1 || true)
(cd "$MAPSERVICE_DIR" && docker compose -f docker-compose-mongo.yaml down >/dev/null 2>&1 || true)

echo "Stopped CaBot local stack"
